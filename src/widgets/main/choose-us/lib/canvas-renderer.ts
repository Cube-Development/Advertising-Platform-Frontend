...
let _hubGlowCache: { cx: number; cy: number; r: number; grad: CanvasGradient } | null = null
...
function drawPlanet(
  ...
  if (!_hubGlowCache || _hubGlowCache.cx !== CX || _hubGlowCache.cy !== CY || _hubGlowCache.r !== hubR) {
    const grad = ctx.createRadialGradient(CX, CY, hubR * 0.5, CX, CY, hubR * 1.6)
...
export function drawConnectionLines(
  ...
  scale: number,
  t: number,
  cards: Array<{ px: number; py: number; connected: boolean; connectTime: number }>,
) {
  const hubR = scale * 0.26
  ...
  cards.forEach((card) => {
    ...
    if (elapsed < FLY_DURATION + 0.3) {
      ...
      if (dist > hubR) {
        ...
        const tailProg = Math.max(0, Math.min((elapsed - 0.1) / (FLY_DURATION + 0.1), 1))
