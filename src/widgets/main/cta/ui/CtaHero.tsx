  ...
  return (
    <div className="flex flex-col items-center gap-4 md:gap-5 lg:items-start">
      <div className="space-y-2 md:space-y-3 lg:space-y-4">
        <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-[44px] font-bold leading-7 text-[#0F2A4D]">
          {title}
        </h2>
      ...
      <div className="sm:block hidden space-y-3 md:space-y-4">
        {features.map(({ Icon, text }, i) => (
          <div key={i} className="flex items-center gap-3 md:gap-4 text-left">
            ...
            <span className="text-sm sm:text-base text-[#000] font-medium">
              {text}
            </span>
          </div>
        ))}
