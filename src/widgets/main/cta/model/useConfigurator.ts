...
const getCategoryTint = (id: number) => {
  const tints = ['#FFE8DA', '#FFF1D6', '#FCE3F0', '#E0F0FF', '#E5EAFF', '#DEF7E5', '#FFE5E0', '#E0F4F7', '#DEF1FF', '#E5F0FF', '#FFE5EC', '#F5E1FF', '#FFEEC7', '#FFE0E5'];
...
export function useConfigurator() {
  ...
  const filteredCategories = useMemo(
    () =>
      categories.map((c, idx) => ({ ...c, _idx: idx })).filter((c) =>
  ...
  const forecastBonus = useMemo(() => {
    ...
    if (categories.length === 0) return 0
    const avg =
      categories.reduce((acc, cat) => acc + calculateBonus(budgetPercent, cat.avgViewsMln), 0) /
      categories.length
