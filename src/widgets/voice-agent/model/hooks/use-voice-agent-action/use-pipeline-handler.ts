...
export function usePipelineHandler() {
    ...
    const handlePipeline = useCallback(
        async (intent: IPipelineIntent) => {
            ...
            try {
                if (intent.action === "NAVIGATE") {
                    ...
                    const items = intent.payload.channels || intent.payload.channelIds || intent.payload.ids;
                    if (items && Array.isArray(items)) {
                        console.log("Voice Agent: Executing ADD_TO_CART with items:", items);
                    ...
                    if (items && Array.isArray(items)) {
                        console.log("Voice Agent: Executing REMOVE_FROM_CART with items:", items);
                        await handleRemoveFromCart(items);
                    }
                } else if (intent.action === "SET_FIELD" || intent.action === "GENERATE_TEXT") {
                    console.log("Voice Agent: SET_FIELD/GENERATE_TEXT action received", intent.payload);

                    const field = intent.payload.field || (intent.action === "GENERATE_TEXT" ? "postText" : "");
                    ...
                    if (field === "postText" && value) {
                        ...
                        console.log("Voice Agent: Setting schedule via Redux bridge", value);
        ...
        [sendAck, handleNavigate, handleAddToCart, handleRemoveFromCart, updateProjectName, isAuth, role, createPostAdvertiser, createPostManager],
    );
