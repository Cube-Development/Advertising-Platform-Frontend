...
import { CATALOG, authApi, baseApi } from "@shared/api";
...
export const catalogAuthAPI = authApi.injectEndpoints({
  endpoints: (build) => ({
    getAuthCatalog: build.query<ICatalogCards, getCatalogReq>({
      query: (BodyParams) => ({
        url: "/sample/",
        method: "POST",
        body: BodyParams,
...
export const { useGetAuthCatalogQuery } = catalogAuthAPI;
